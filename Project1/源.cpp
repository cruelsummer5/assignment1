...
int count_empty_lines(const string& filename)
{
...
{
	return entry.path().extension() == ".cc" || entry.path().extension() == ".cpp" || entry.path().extension() == ".cxx" || entry.path().extension() == ".C" || entry.path().extension() == ".c++";
}
...
void count_cpp_files(const fs::path& dir, DirCxxBreif& dcb) 
{
	for (const auto& entry : fs::directory_iterator(dir)) 
	{
		...
		if (isCppFile) 
		{
			...
			int lines = count_lines(entry.path().string());
			dcb.lineCount += lines;

			int emptyLines = count_empty_lines(entry.path().string());
			dcb.emptyLineCount += emptyLines;

			cout << entry.path() << ": " << lines << " lines, " << emptyLines << " empty lines\n";
		}
	}
}
